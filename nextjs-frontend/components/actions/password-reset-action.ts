"use server";

import { redirect } from "next/navigation";
import { passwordResetConfirmSchema } from "@/lib/definitions";
import { confirmPasswordReset, requestPasswordReset } from "@/lib/auth";

export async function passwordReset(prevState: unknown, formData: FormData) {
  const input = {
    body: {
      email: formData.get("email") as string,
    },
  };

  try {
    const result = await requestPasswordReset(input.body.email);
    if (!result.ok) {
      return { server_validation_error: result.message };
    }
    return { message: "Instruksi reset password sudah dikirim ke email Anda." };
  } catch (err) {
    console.error("Password reset error:", err);
    return {
      server_error: "An unexpected error occurred. Please try again later.",
    };
  }
}

export async function passwordResetConfirm(
  prevState: unknown,
  formData: FormData,
) {
  const validatedFields = passwordResetConfirmSchema.safeParse({
    token: formData.get("resetToken") as string,
    password: formData.get("password") as string,
    passwordConfirm: formData.get("passwordConfirm") as string,
  });

  if (!validatedFields.success) {
    return {
      errors: validatedFields.error.flatten().fieldErrors,
    };
  }

  const { token, password } = validatedFields.data;
  const input = {
    body: {
      token,
      password,
    },
  };
  try {
    const result = await confirmPasswordReset(input.body);
    if (!result.ok) {
      return { server_validation_error: result.message };
    }
    redirect(`/login`);
  } catch (err) {
    console.error("Password reset confirmation error:", err);
    return {
      server_error: "An unexpected error occurred. Please try again later.",
    };
  }
}
